...
class Pager {
     ...
     * @param INT $Total Informe o total de posts para returnPage recursivo!
     */
    function __construct($Link, $First = null, $Last = null, $MaxLinks = null, $Total = null) {
        ...
        $this->Total = ( (int) $Total ? $Total : null);
    }
    ...
    public function ReturnPage() {
        if ($this->Page > 1):
            ...
            if (!$this->Total):
                ...
                $nPage = ceil($this->Total / $this->Limit);
                header("Location: {$this->Link}{$nPage}");
                exit;
            endif;
            die;
    ...
    /**
     * <b>Executar Paginação:</b> Cria o menu de navegação de paginação dentro de uma lista não ordenada com a class paginator.
     * Informe a string completa da Query e o ParseString. O resto é feito pelo método. Execute um <b>echo getPaginator();</b>
     * para exibir a paginação na view.
     * @param STRING $Tabela = Nome da tabela
     * @param STRING $Termos = Condição da seleção caso tenha
     * @param STRING $ParseString = Prepared Statements
     * @param STRING $Hash = #urlhash
     */
    public function ExeFullPaginator($Query, $ParseString = null, $Hash = null) {
